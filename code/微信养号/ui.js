...
function save_config(fun){
    ...
    var storage = storages.create("hm5p-多-667")
    if(fun=="加载"){
        ui.设备编号.setText(       storage.get("设备编号"  )      )
        ui.定制版本.setText(       storage.get("定制版本"  )      )
        ui.软件端口1.setText(      storage.get("软件端口1" )      )
        ui.软件端口2.setText(      storage.get("软件端口2" )      )
        ui.随机跑.setText(         storage.get("随机跑"    )      )
        ui.循环量.setText(         storage.get("循环量"    )      )
        ui.聊会天_input.setText(   storage.get("聊会天_input"    )      )
        ui.腾新闻_input.setText(   storage.get("腾新闻_input"    )      )
        ui.发文字_input.setText(   storage.get("发文字_input"    )      )
        ui.发图文_input.setText(   storage.get("发图文_input"    )      )
        ui.加群员_input.setText(   storage.get("加群员_input"    )      )
        ui.加公众_input.setText(   storage.get("加公众_input"    )      )
        ui.搜索加_input.setText(   storage.get("搜索加_input"    )      )
        ui.建微群.setText(         storage.get("建微群"    )      )
        ui.切换网络.setChecked(    storage.get("切换网络"  )      )
        ui.正常顺序.setChecked(    storage.get("正常顺序"  )      )
        ui.超强防封.setChecked(    storage.get("超强防封"  )      )
        ui.聊会天_check.setChecked(      storage.get("聊会天_check"    )      )
        ui.没人聊.setChecked(      storage.get("没人聊"    )      )
        ui.腾新闻.setChecked(      storage.get("腾新闻"    )      )
        ui.点赞量.setChecked(      storage.get("点赞量"    )      )
        ui.发文字.setChecked(      storage.get("发文字"    )      )
        ui.发图文.setChecked(      storage.get("发图文"    )      )
        ui.加群员.setChecked(      storage.get("加群员"    )      )
        ui.加微群.setChecked(      storage.get("加微群"    )      )
        ui.搜索加.setChecked(      storage.get("搜索加"    )      )
        ui.通讯加.setChecked(      storage.get("通讯加"    )      )
        ui.加公众.setChecked(      storage.get("加公众"    )      )
        ui.固通讯.setChecked(      storage.get("固通讯"    )      )
        ui.发新闻.setChecked(      storage.get("发新闻"    )      )
        ui.功能选择.setSelection(  storage.get("功能选择"  )      )
