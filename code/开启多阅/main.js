...
function jiance() {
    ...
    while(true){
        ...
        if (qi_dong_gong_neng) {
            log("发现启动功能,正在检测是否有更新窗口")
            start_flg+=1
            sleep(1000)
        }
        if (qi_dong_gong_neng && start_flg>7) {
            // sleep(1000)
            log("启动功能已点击")
...
function start_duoyue2() {
    sh.press(710,385,100)
    sleep(3000)
    sh.press(370,405,100)
    log("点击运行完成")
}
...
function start_duoyue() {
    ...
    sh.press(auto.root.child(0).child(0).bounds().centerX(),auto.root.child(0).child(0).bounds().centerY(),100)
    sleep(200)
