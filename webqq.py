...
url = 'https://ssl.ptlogin2.qq.com/check?uin=' + \
    str(self.user) + '&appid=1003903&js_ver=10017&js_type=0&login_sig=0ihp3t5ghfoonssle-98x9hy4uaqmpvu*8*odgl5vyerelcb8fk-y3ts6c3*7e8-&u1=http%3A%2F%2Fweb2.qq.com%2Floginproxy.html&r=0.8210972726810724'
...
if self.check == "1":
url = 'https://ssl.captcha.qq.com/getimage?&uin=' + \
    str(self.user)+'&aid=1002101&0.45644426648505' + str(random.randint(10, 99))
req = urllib2.Request(url)
req = urllib2.urlopen(req)
...
data = 'r=%7B%22status%22%3A%22online%22%2C%22ptwebqq%22%3A%22' + self.ptwebqq + '%22%2C%22passwd_sig%22%3A%22%22%2C%22clientid%22%3A%22' + \
...
data = 'r=%7B%22clientid%22%3A%22'+self.clientid+'%22%2C%22psessionid%22%3A%22' + \
    self.result['result']['psessionid']+'%22%2C%22key%22%3A0%2C%22ids%22%3A%5B%5D%7D&clientid=' + \
    self.clientid+'&psessionidpsessionid='+self.result['result']['psessionid']
req = urllib2.Request(url, data)
#req.add_header('Cookie', self.mycookie)
req.add_header(
...
data = 'r=%7B%22to%22%3A'+uin+'%2C%22face%22%3A237%2C%22content'+urllib.quote(r'":"[\"'+msg+r'\",\"\\n【提示：此用户正在使用shift webQq】\",[\"font\",{\"name\":\"宋体\",\"size\":\"10\",\"style\":[0,0,0],\"color\":\"000000\"}]]","') + \
    'msg_id%22%3A13190001%2C%22clientid%22%3A%22'+self.clientid+'%22%2C%22psessionid%22%3A%22' + \
    self.result['result']['psessionid']+'%22%7D&clientid='+self.clientid + \
    '&psessionidpsessionid='+self.result['result']['psessionid']
req = urllib2.Request(url, data)
#req.add_header('Cookie', self.mycookie)
req.add_header(
    'Referer', 'http://d.web2.qq.com/proxy.html?v=20110331002&callback=1&id=2')
